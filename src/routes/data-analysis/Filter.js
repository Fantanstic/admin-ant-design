...
const Filter = ({
  ...
  const handleFields = (fields) => {
    const { date } = fields
    if (date.length) {
      fields.date = [date[0].format('YYYY-MM-DD'), date[1].format('YYYY-MM-DD')]
    }
    return fields
  }
  ...
  if (filter.date && filter.date[0]) {
    initialCreateTime[0] = moment(filter.date[0])
  }
  if (filter.date && filter.date[1]) {
    initialCreateTime[1] = moment(filter.date[1])
  }
    ...
    <Row gutter={24}>
      <Col {...ColProps} xl={{ span: 6 }} md={{ span: 8 }} sm={{ span: 12 }} id="createTimeRangePicker">
        <FilterItem label="Createtime">
          {getFieldDecorator('date', { initialValue: initialCreateTime })(<RangePicker
            style={{ width: '100%' }}
